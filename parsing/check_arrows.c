...
/*   Updated: 2023/04/17 10:45:19 by mfinette         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_arrow_symbols(t_msh *msh, int i, char c)
{
	...
	while (msh->line[i] && is_white_space(msh->line[i]))
		i++;
	if (count == 3)
		return (g_status = 2, printf("%s `%c'\n", ERR_MESS, c), -1);
	else if (count == 4)
		return (g_status = 2, printf("%s `%c%c'\n", ERR_MESS, c, c), -1);
	else if (count >= 5)
		return (g_status = 2, printf("%s `%c%c%c'\n", ERR_MESS, c, c, c), -1);
	else if (msh->line[i] == '|')
		return (g_status = 2, printf("%s `|'\n", ERR_MESS), -1);
	else if (msh->line[i] == '\0')
		return (g_status = 2, printf("%s `newline'\n", ERR_MESS), -1);
